...
// #include "dd/ContinuousVector.hpp"
...
const int const_zero_edge = -1;
const int const_one_node = -1;
...
public:
  ...
  int nodeCount( Edge<mNode>& x ) {
    std::map<mNode*, bool> visited;

    int result = 0;

    std::function<int (mNode* node)> nodeNum = [&] (mNode* node) {
      int node_cnt = 0;
      ...
      if (visited[node] == false) {
        ...
        for (int i = 0; i < rows; ++i) {
          for (int j = 0; j < cols; j++)
          {
            ...
            {
              node_cnt += nodeNum(node->e[i*cols+j].p);
            }
...
public:
  ...
  template <class Edge> std::size_t node_count(const Edge& e) {
    static constexpr std::size_t NODECOUNT_BUCKETS = 200000U;
    static std::unordered_set<decltype(e.p)> visited{NODECOUNT_BUCKETS}; // 2e6
    visited.max_load_factor(10);
    visited.clear();
    return nodeCountExclusionary(e, visited);
  }
  template <class Edge> std::size_t edge_count(const Edge& e) {
    static constexpr std::size_t NODECOUNT_BUCKETS = 200000U;
    static std::unordered_set<decltype(e.p)> visited{NODECOUNT_BUCKETS}; // 2e6
    visited.max_load_factor(10);
    visited.clear(); 
    return edgeCount(e, visited);
  }

  void DD_matrix_extract(
    const mEdge& e, 
    std::vector<std::vector<int>> &tgt_qubits_gates, 
    std::vector<std::vector<ComplexValue>> &tensor_gates
  ) {
    std::vector<int> num_node_vec(e.p->v+1, 0);
    std::vector<bool> ident_vec(e.p->v+1, false);
    static constexpr std::size_t NODECOUNT_BUCKETS = 200000U;
    static std::unordered_set<decltype(e.p)> visited{NODECOUNT_BUCKETS}; // 2e6
    visited.max_load_factor(10);
    visited.clear();
    num_node_per_qubit(e, num_node_vec, visited);
    visited.clear();
    ident_per_qubit(e, num_node_vec, ident_vec, visited);
    int tensor_size_pow = 0;
    std::vector<int> tgt_qubits;
    std::vector<size_t> seg_vec(e.p->v+1, 0);
    int num_tgts = 0;
    for (int i = 0; i < ident_vec.size(); i++) {
      if (!ident_vec[i]) {
        tensor_size_pow++;
        tgt_qubits.push_back(i);
        seg_vec[i] = std::pow(2, num_tgts);
        num_tgts++;
      }
    }
    tgt_qubits_gates.push_back(tgt_qubits);
    size_t tensor_size = std::pow(2, tensor_size_pow);
    std::vector<ComplexValue> tensor(tensor_size*tensor_size, {0, 0});

    dfs_tensor_fill(e, tensor, seg_vec, 0, 0, {RealNumber::val(e.w.r), RealNumber::val(e.w.i)}, tensor_size);
    tensor_gates.push_back(tensor);
  }

  void dfs_tensor_fill(
    const mEdge& e, 
    std::vector<ComplexValue> &tensor,
    std::vector<size_t> seg_vec,
    size_t row_offset,
    size_t col_offset,
    ComplexValue factor,
    size_t tensor_size
    ...
    if (e.isTerminal()) {
      tensor[row_offset*tensor_size+col_offset] = factor;
      return;
    }
    if (seg_vec[e.p->v] == 0) {
      const auto ar = RealNumber::val(e.p->e[0].w.r);
      const auto ai = RealNumber::val(e.p->e[0].w.i);
      ComplexValue nxt_factor{ar * factor.r - ai * factor.i,
                      ar * factor.i + ai * factor.r};
      dfs_tensor_fill(e.p->e[0], tensor, seg_vec, row_offset, col_offset, factor, tensor_size);
      ...
      {
        for (int j = 0; j < 2; j++) {
          if (!(e.p->e[i*2+j].w.exactlyZero())) {
            const auto ar = RealNumber::val(e.p->e[i*2+j].w.r);
            const auto ai = RealNumber::val(e.p->e[i*2+j].w.i);
            ComplexValue nxt_factor{ar * factor.r - ai * factor.i,
                            ar * factor.i + ai * factor.r};
            ...
            dfs_tensor_fill(
              ...
              tensor,
              seg_vec,
              row_offset+i*seg_vec[e.p->v],
              col_offset+j*seg_vec[e.p->v],
              nxt_factor,
              tensor_size
            );
          }
  ...
  void num_node_per_qubit(const mEdge& e, 
    std::vector<int> &num_node_vec,
    std::unordered_set<decltype(e.p)>& v
  ) {
    v.insert(e.p);
    if (!e.isTerminal()) {

      num_node_vec[e.p->v] += 1;
      for (const auto& edge : e.p->e) {
        if (!v.count(edge.p)) {
          if (!(edge.w.exactlyZero()))
            num_node_per_qubit(edge, num_node_vec, v);
        }
      }
    }
  }
  
  void ident_per_qubit(const mEdge& e, 
    std::vector<int> &num_node_vec,
    std::vector<bool> &ident_vec,
    std::unordered_set<decltype(e.p)>& v
  ) {
    v.insert(e.p);
    if (!e.isTerminal()) {
      if (num_node_vec[e.p->v] == 1) {
        if (e.p->e[0] == e.p->e[3] && e.p->e[1].w.exactlyZero() && e.p->e[2].w.exactlyZero()) {
          ident_vec[e.p->v] = true;
        }
      }
      for (const auto& edge : e.p->e) {
        if (!v.count(edge.p)) {
          if (!(edge.w.exactlyZero()))
            ident_per_qubit(edge, num_node_vec, ident_vec, v);
        }
      }
    }
  }

  void DFS_fill_gpu_structure(const mEdge& e, 
    GPU_DD_edge* gpu_edge_arr, GPU_DD_node* gpu_node_arr) 
  {
    static constexpr std::size_t NODECOUNT_BUCKETS = 200000U;
    static std::unordered_set<decltype(e.p)> visited{NODECOUNT_BUCKETS}; // 2e6
    visited.max_load_factor(10);
    ...
    std::unordered_map<mNode*, int> node_id_map;
    DFS_structure(e, gpu_edge_arr, gpu_node_arr, visited, node_id_map, edge_ptr, node_stack_ptr);
  }
  ...
  void dd_extract_matrix_cpu(const mEdge& e, cuDoubleComplex *fused_gate_val_h, int *fused_gate_indices_h, size_t idx_x, size_t idx_y, int *sparse_idx_x, int num_mac, cuDoubleComplex factor) {
    const auto ar = RealNumber::val(e.w.r);
    const auto ai = RealNumber::val(e.w.i);
    cuDoubleComplex nxt_factor{ar * factor.x - ai * factor.y,
                    ar * factor.y + ai * factor.x};
    if (e.isTerminal()) {
      int sparse_idx_term = sparse_idx_x[idx_y];
      fused_gate_indices_h[num_mac*idx_y+sparse_idx_term] = idx_x;
      sparse_idx_x[idx_y]++;
      fused_gate_val_h[num_mac*idx_y+sparse_idx_term] = nxt_factor; // 
      return;
    }
    for (int i = 0; i < 2; i++) {
      for (int j = 0; j < 2; j++) {
        if (!(e.p->e[i*2+j].w.exactlyZero())) {
          dd_extract_matrix_cpu(e.p->e[i*2+j], fused_gate_val_h, fused_gate_indices_h, 
          idx_x+j*std::pow(2, e.p->v), idx_y+i*std::pow(2, e.p->v), sparse_idx_x, num_mac, nxt_factor);
        }
      }
    }
  ...
  std::size_t MaxELLRowDD(const mEdge& e, 
    std::unordered_map<mNode*, vEdge> &mac_map) 
  {
    vEdge dd_vec = MaxELLRowDDRecur(e, mac_map);
    static constexpr std::size_t NODECOUNT_BUCKETS = 200000U;
    static std::unordered_set<decltype(dd_vec.p)> visited{NODECOUNT_BUCKETS}; // 2e6
    visited.max_load_factor(10);
    visited.clear();
    int node_num = nodeCount(dd_vec, visited);
    // new experiment
    // int max_value = std::round(DDMaxValue(dd_vec));
    // int min_value = std::round(DDMinValue(dd_vec));
    // if (max_value != min_value) 
    // {
    //   std::vector<int> nzrvals;
    //   double mean = 0;
    //   for (int i = 0; i < (1ULL << dd_vec.p->v); i++)
    //   {
    //     int nzr = getValueByPath(dd_vec, i).r;
    //     nzrvals.emplace_back(nzr);
    //     mean += nzr;
    //   }
    //   mean = mean / (1ULL << dd_vec.p->v);

    //   double sd = 0;
    //   for (int i = 0; i < (1ULL << dd_vec.p->v); i++)
    //   {
    //     sd += ((double)nzrvals[i]-mean) * ((double)nzrvals[i]-mean);
    //   }
    //   sd = sd / (1ULL << dd_vec.p->v);
    //   sd = sqrt(sd);
    //   double cv = sd / (double)mean;
    //   printf(" Coefficient of variation = %f\n",cv);
    //   cv_accum += cv;
    // }
    fuse_cnt++;
    // end new experiment
    return std::round(DDMaxValue(dd_vec));
  }

  vEdge MaxELLRowDDRecur(const mEdge& e, 
    std::unordered_map<mNode*, vEdge> &mac_map) 
  {
    if (e.isTerminal()) {
      return vEdge::one;
    ...
    if (mac_map.find(e.p) == mac_map.end()) {
      ...
      vEdge a2 = vEdge::zero;
      vEdge a3 = vEdge::zero;
      vEdge a01 = vEdge::zero;
      vEdge a23 = vEdge::zero;
      std::array<vEdge, 2> edge{};
      vEdge A = vEdge::zero;

      if (!(e.p->e[0].w.exactlyZero())) {
        a0 = MaxELLRowDDRecur(e.p->e[0], mac_map);
      }
      if (!(e.p->e[1].w.exactlyZero())) {
        a1 = MaxELLRowDDRecur(e.p->e[1], mac_map);
      }
      if (!(e.p->e[2].w.exactlyZero())) {
        a2 = MaxELLRowDDRecur(e.p->e[2], mac_map);
      }
      if (!(e.p->e[3].w.exactlyZero())) {
        a3 = MaxELLRowDDRecur(e.p->e[3], mac_map);
      }
      edge[0] = (add2(a0, a1));
      edge[1] = (add2(a2, a3));
      
      A = makeDDNode(e.p->v, edge, true);
      mac_map.insert({ e.p, A });
      return A;
    } else {
      return mac_map[e.p];
    }
  ...
  dd::fp DDMaxValue(const vEdge& e) {
    if (e.isTerminal()) {
      return RealNumber::val(e.w.r);
    }
    dd::fp left = 0;
    dd::fp right = 0;
    if (!e.p->e[0].w.exactlyZero()) {
      left = DDMaxValue(e.p->e[0]);
    }
    if (!e.p->e[1].w.exactlyZero()) {
      right = DDMaxValue(e.p->e[1]);
    }
    return RealNumber::val(e.w.r) * std::max(left, right);
  }

  dd::fp DDMinValue(const vEdge& e) {
    if (e.isTerminal()) {
      return RealNumber::val(e.w.r);
    }
    dd::fp left = 0;
    dd::fp right = 0;
    if (!e.p->e[0].w.exactlyZero()) {
      left = DDMinValue(e.p->e[0]);
    }
    if (!e.p->e[1].w.exactlyZero()) {
      right = DDMinValue(e.p->e[1]);
    }
    return RealNumber::val(e.w.r) * std::min(left, right);
  }

  // std::size_t MaxELLRow(const mEdge& e, 
  //   std::unordered_map<mNode*, ContinuousVector> &mac_map, 
  //   std::size_t offset0, std::size_t offset1) {
  //   auto vec = MaxELLRowRecur(e, mac_map, offset0, offset1);
  //   return vec.Max();
  // }

  // ContinuousVector MaxELLRowRecur(const mEdge& e, 
  //   std::unordered_map<mNode*, ContinuousVector> &mac_map, 
  //   std::size_t offset0, std::size_t offset1) {
  //   if (e.isTerminal()) {
  //     return ContinuousVector(1, {0}, {1});
  //   }
  ...
  //     ContinuousVector a0 = ContinuousVector(mid_offset-offset0, {0},{0} );
  //     ContinuousVector a1 = ContinuousVector(offset1-mid_offset, {0},{0} );
  //     ContinuousVector a2 = ContinuousVector(mid_offset-offset0, {0},{0} );
  //     ContinuousVector a3 = ContinuousVector(offset1-mid_offset, {0},{0} );
  //     ContinuousVector a01 = ContinuousVector(mid_offset-offset0, {0},{0} );
  //     ContinuousVector a23 = ContinuousVector(offset1-mid_offset, {0},{0} );
  //     ContinuousVector A = ContinuousVector(offset1-offset0, {0},{0} );

  //     if (!(e.p->e[0].w.exactlyZero())) {
  //       a0 = MaxELLRowRecur(e.p->e[0], mac_map, offset0, mid_offset);
  //     }
  //     if (!(e.p->e[1].w.exactlyZero())) {
  //       a1 = MaxELLRowRecur(e.p->e[1], mac_map, mid_offset, offset1);
  //     }
  //     if (!(e.p->e[2].w.exactlyZero())) {
  //       a2 = MaxELLRowRecur(e.p->e[2], mac_map, offset0, mid_offset);
  //     }
  //     if (!(e.p->e[3].w.exactlyZero())) {
  //       a3 = MaxELLRowRecur(e.p->e[3], mac_map, mid_offset, offset1);
  //     }

  //     a01.CVAdd(a0, a1);
  //     a23.CVAdd(a2, a3);
  //     A.CVConcatenate(a01, a23);
  //     mac_map.insert({ e.p, A });
  //     return A;
  //   }
  //   else {
  ...
  std::size_t MaxMACRow(const mEdge& e, 
    std::unordered_map<mNode*, std::size_t> &mac_map, 
    std::size_t offset0, std::size_t offset1
  ) {
    if (e.isTerminal()) {
    ...
    if (mac_map.find(e.p) == mac_map.end()) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        std::size_t mid_offset = (offset0+offset1) / 2;
        if (!(e.p->e[i*2].w.exactlyZero())) {
          local_cnt += MaxMACRow(e.p->e[i*2], mac_map, offset0, mid_offset);
        }
        if (!(e.p->e[i*2+1].w.exactlyZero())) {
          local_cnt += MaxMACRow(e.p->e[i*2+1], mac_map, mid_offset, offset1);
        }
        if (local_cnt > cnt) {
          cnt = local_cnt;
        }
...
private:
  void DFS_structure(const mEdge& e, 
    GPU_DD_edge* gpu_edge_arr, GPU_DD_node* gpu_node_arr, 
    std::unordered_set<decltype(e.p)>& v, std::unordered_map<mNode*, int> &node_id_map,
    int &edge_ptr, int &node_stack_ptr) 
  {
    if (!e.isTerminal()) {
      if (!v.count(e.p)) {
        ...
        gpu_edge_arr[edge_ptr] = {{RealNumber::val(e.w.r), RealNumber::val(e.w.i)}, node_id};
        edge_ptr++;
        ...
        for (const auto& edge : e.p->e) {
          // sth. TODO
          if (edge.w.exactlyZero()) {
            gpu_node_arr[node_id].outgoing_DD_edge_ptr[edge_idx] = const_zero_edge;
          }
          else {
            gpu_node_arr[node_id].outgoing_DD_edge_ptr[edge_idx] = edge_ptr;
            // if (!v.count(edge.p)) 
            {
              DFS_structure(edge, gpu_edge_arr, gpu_node_arr, v, node_id_map,
                edge_ptr, node_stack_ptr
              );
            }
          }
      ...
      gpu_edge_arr[edge_ptr] = {{RealNumber::val(e.w.r), RealNumber::val(e.w.i)}, const_one_node};
      edge_ptr++;
    }
  }
  ...
  template <class Edge>
  std::size_t nodeCountExclusionary(const Edge& e,
                        std::unordered_set<decltype(e.p)>& v) const {
    v.insert(e.p);
    std::size_t sum = !e.isTerminal();
    if (!e.isTerminal()) {
      for (const auto& edge : e.p->e) {
        if (!v.count(edge.p)) {
          sum += nodeCountExclusionary(edge, v);
        }
      }
    }
    return sum;
  ...
  template <class Edge>
  std::size_t edgeCount(const Edge& e,
                        std::unordered_set<decltype(e.p)>& v) const {
    
    std::size_t sum = (!(e.w.exactlyZero()));
    if (!e.isTerminal() && !v.count(e.p)) {
      v.insert(e.p);
      for (const auto& edge : e.p->e) {
        sum += edgeCount(edge, v);
      }
    }
    return sum;
  }
