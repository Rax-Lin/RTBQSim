...
resolve_cuda_arch() {
  if [[ -n "${CMAKE_CUDA_ARCHITECTURES:-}" ]]; then
    echo "[cuquantum_compile.sh] Using user-provided CMAKE_CUDA_ARCHITECTURES=${CMAKE_CUDA_ARCHITECTURES}" >&2
  ...
  if [[ -n "${gpu_arch}" && ${#supported_arches[@]} -gt 0 ]]; then
    ...
    if [[ -z "${selected}" ]]; then
      ...
      echo "[cuquantum_compile.sh] GPU compute capability (${gpu_cc}) is below nvcc minimum; using ${selected}." >&2
    elif [[ "${selected}" != "${gpu_arch}" ]]; then
      echo "[cuquantum_compile.sh] GPU compute capability (${gpu_cc}) not directly supported by nvcc; using compatible arch ${selected}." >&2
    else
      echo "[cuquantum_compile.sh] Auto-detected CUDA arch ${selected} from GPU compute capability ${gpu_cc}." >&2
    fi
  ...
  if [[ ${#supported_arches[@]} -gt 0 ]]; then
    ...
    echo "[cuquantum_compile.sh] Could not query GPU compute capability; using highest nvcc-supported arch ${selected}." >&2
    printf '%s\n' "${selected}"
    return 0
  fi

  echo "[cuquantum_compile.sh] Could not auto-detect CUDA arch; fallback to 86." >&2
  printf '%s\n' "86"
}
...
if [[ "${CUDA_ARCH}" == "87" ]]; then
  echo "[cuquantum_compile.sh] OptiX desktop compatibility fallback: sm_87 -> sm_86." >&2
  CUDA_ARCH="86"
fi
...
if ! command -v nvcc >/dev/null 2>&1; then
  echo "[cuquantum_compile.sh] nvcc was not found in PATH. Please compile inside the CUDA-enabled docker/environment." >&2
fi
...
if [[ -f "${BUILD_DIR}/CMakeCache.txt" ]]; then
  ...
  if [[ -n "${CACHE_SRC}" && "${CACHE_SRC}" != "${ROOT_DIR}" ]]; then
    ...
    echo "[cuquantum_compile.sh] CUDA arch changed (${CACHE_ARCH} -> ${CUDA_ARCH}). Cleaning ${BUILD_DIR}."
    clear_build_dir
  fi
fi
