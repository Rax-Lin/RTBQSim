...
void printResult( optixState& state, const std::string& fileName, const float* host_result = nullptr )
{
    // Open File
    ...
    // Copy result to host if not provided
    float* result = const_cast<float*>(host_result);
    bool owns_result = false;
    if (result == nullptr) {
        result = (float*)malloc(state.d_result_buf_size);
    ...
    // First pass: count NNZ to avoid header overwrite risks
    for (int i = 0; i < numRows; ++i) {
        for (int j = 0; j < numCols; ++j) {
            uint64_t idx = static_cast<uint64_t>(i) * static_cast<uint64_t>(numCols) + static_cast<uint64_t>(j);
            if (result[idx] == 0) {
                continue;
            }
            ++nonZeroCount;
...
int main( int argc, char* argv[] )
{
    ...
    for( int i = 1; i < argc; ++i )
    {
        ...
        else if( arg == "--output" || arg == "-o" )
        {
            ...
            if (i < argc - 1) matrix1File = argv[++i];
            else printUsageAndExit(argv[0]);
        }
        else if (arg == "--mat2" || arg == "-m2") {
            if (i < argc - 1) matrix2File = argv[++i];
            else printUsageAndExit(argv[0]);
        }
        else if (arg == "--log" || arg == "-l") {
            if (i < argc - 1) logFile = argv[++i];
            else printUsageAndExit(argv[0]);
        }
    }
    ...
    try
    {
        ...
        contextSetUp( state ); // Context 必須先建立，後續 Load Data 才能使用
        storeSphereData(state, matrix2File); 
        ...
        // ==========================================================================
        // STAGE 2: Work Estimation (BVH Build & Pipeline Setup)
        // ==========================================================================
        ...
        // ==========================================================================
        // STAGE 3: Actual Computation (Launch)
        // ==========================================================================
        CUstream stream;
        ...
        CUDA_CHECK( cudaMemcpy( reinterpret_cast<void*>( d_param ), &state.params, sizeof( state.params ), cudaMemcpyHostToDevice ) );

        uint64_t numRayLaunch = state.d_size;
        CUDA_CHECK( cudaEventRecord( start, stream ) );
        OPTIX_CHECK( optixLaunch( state.pipeline, stream, d_param, sizeof( Params ), &state.sbt, numRayLaunch, 1, 1 ) );
        CUDA_CHECK( cudaEventRecord( stop, stream ) );
        CUDA_CHECK( cudaEventSynchronize( stop ) );
        CUDA_CHECK( cudaEventElapsedTime( &compute_ms, start, stop ) );

        // ==========================================================================
        // STAGE 4: D2H Transfer
        // ==========================================================================
        float* h_temp_result = (float*)malloc(state.d_result_buf_size);
        CUDA_CHECK( cudaEventRecord( start ) );

        // 為了精確計時傳輸，將原本 printResult 內的 memcpy 提取到此處
        CUDA_CHECK( cudaMemcpy(h_temp_result, state.d_result, state.d_result_buf_size, cudaMemcpyDeviceToHost) );
        ...
        // ==========================================================================
        // 輸出結果至 logFile (對接 Python AE_test.py)
        // ==========================================================================
        if (!logFile.empty()) {
            std::ofstream log(logFile);
            if (log.is_open()) {
                log << "H2D_Transfer," << h2d_ms << "\n";
                log << "Work_Estimation," << work_est_ms << "\n";
                log << "Actual_Compute," << compute_ms << "\n";
                log << "D2H_Transfer," << d2h_ms << "\n";
                log.close();
            }
        }

        // 顯示原本的成功訊息與輸出檔案 (沿用已搬回的結果避免重複 memcpy)
        printResult(state, outfile, h_temp_result);
        free(h_temp_result);
        printf("success!");
